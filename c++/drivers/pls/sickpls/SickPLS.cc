...
namespace SickToolbox {
								...
								_sick_values_subrange_stop_index(0)
  {
    ...
    memset(&_sick_restart_status,0,sizeof(sick_pls_restart_status_t));
    memset(&_sick_pollution_status,0,sizeof(sick_pls_pollution_status_t));
    memset(&_sick_signal_status,0,sizeof(sick_pls_signal_status_t));
    memset(&_sick_field_status,0,sizeof(sick_pls_field_status_t));
   ...
   */
  void SickPLS::SetSickMeasuringUnits( const sick_pls_measuring_units_t sick_units )
    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::SetSickMeasuringUnits: Sick PLS is not initialized!");
    }

    /* Ensure a valid units type was given */
    if (!_validSickMeasuringUnits(sick_units)) {
      throw SickConfigException("SickPLS::SetSickMeasuringUnits: Undefined measurement units!");
    }
    ...
    if (sick_units != _sick_device_config.sick_measuring_units) {
      ...
      sick_pls_device_config_t sick_device_config;
	...
	throw;
      ...
      /* Handle anything else */
      	...
      	std::cerr << "SickPLS::SetSickMeasuringUnits: Unknown exception!!!" << std::endl;
      	throw;
      }

    }
    else {
      std::cerr << "\tSickPLS::SetSickMeasuringUnits - Device is already configured w/ these units. (skipping write)" << std::endl;
    }
  ...
  /**
   * \brief Sets the Sick PLS sensitivity level
   * \param sick_sensitivity Desired sensitivity level
   */
  void SickPLS::SetSickSensitivity( const sick_pls_sensitivity_t sick_sensitivity )
    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::SetSickSensitivity: Sick PLS is not initialized!");
    }
    
    /* Ensure the command is supported by the given Sick model */
    if (!_isSickPLS211() && !_isSickPLS221() && !_isSickPLS291()) {
      throw SickConfigException("SickPLS::SetSickSensitivity: This command is not supported by this Sick model!");
    }

    /* Ensure this is a valid sick sensitivity value*/
    if (!_validSickSensitivity(sick_sensitivity)) {
      throw SickConfigException("SickPLS::SetSickSensitivity: Undefined sensitivity level!");
    }
	...
	throw;
      ...
      /* Handle anything else */
      	...
      	std::cerr << "SickPLS::SetSickSensitivity: Unknown exception!!!" << std::endl;
      	throw;
      }

    }
    else {
      std::cerr << "\tSickPLS::SetSickSensitivity - Sick is already operating at this sensitivity level! (skipping write)" << std::endl;
    }
   ...
   * \param sick_sensitivity Desired sensitivity level
   */
  void SickPLS::SetSickPeakThreshold( const sick_pls_peak_threshold_t sick_peak_threshold )
    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::SetSickPeakThreshold: Sick PLS is not initialized!");
    }
    
    /* Ensure the command is supported by the given Sick model */
    if (!_isSickPLS200() && !_isSickPLS220()) {
      throw SickConfigException("SickPLS::SetSickPeakThreshold: This command is not supported by this Sick model!");
    }
    
    /* Ensure this is a valid sick sensitivity value*/
    if (!_validSickPeakThreshold(sick_peak_threshold)) {
      throw SickConfigException("SickPLS::SetSickPeakThreshold: Undefined peak threshold!");
    }
	...
	throw;
      ...
      /* Handle anything else */
      	...
      	std::cerr << "SickPLS::SetSickPeakThreshold: Unknown exception!!!" << std::endl;
      	throw;
      }

    }
    else {
      std::cerr << "\tSickPLS::SetSickPeakThreshold - Sick is already operating w/ given threshold! (skipping write)" << std::endl;
    }
    
  }
  
  /**
   * \brief Gets the current Sick PLS sensitivity level
   * \return Sensitivity level (sick_pls_sensitivity_t)
   */
  sick_pls_sensitivity_t SickPLS::GetSickSensitivity( ) const throw( SickConfigException ) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::GetSickSensitivity: Sick PLS is not initialized!");
    }
    ...
    return (sick_pls_sensitivity_t)_sick_device_config.sick_peak_threshold;  //If the device is 211/221/291 then this value is sensitivity
   ...
   * \return Sensitivity level (sick_pls_sensitivity_t)
   */
  sick_pls_peak_threshold_t SickPLS::GetSickPeakThreshold( ) const throw( SickConfigException ) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::GetSickPeakThreshold: Sick PLS is not initialized!");
    }
    ...
    /* If its supported than return the actual value */
    return (sick_pls_peak_threshold_t)_sick_device_config.sick_peak_threshold;  //If the device is 211/221/291 then this value is sensitivity
   ...
   */
  void SickPLS::SetSickMeasuringMode( const sick_pls_measuring_mode_t sick_measuring_mode )
    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {
    ...
    if (!_validSickMeasuringMode(sick_measuring_mode)) {
      throw SickConfigException("SickPLS::SetSickMeasuringMode: Undefined measuring mode!");
    }
	...
	throw;
      ...
      /* Handle anything else */
      	...
      	std::cerr << "SickPLS::SetSickMeasuringMode: Unknown exception!!!" << std::endl;
      	throw;
      }

    }
    else {
      std::cerr << "\tSickPLS::SetSickMeasuringMode - Sick is already operating w/ this measuring mode! (skipping write)" << std::endl;
    }
      
  }

  /**
   * \brief Gets the current Sick PLS measuring mode
   * \return Measuring mode (sick_pls_measuring_mode_t)
   */
  sick_pls_measuring_mode_t SickPLS::GetSickMeasuringMode( ) const throw( SickConfigException ) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::GetSickMeasuringMode: Sick PLS is not initialized!");
    }

    /* Return the determined measuring mode */
    return (sick_pls_measuring_mode_t)_sick_operating_status.sick_measuring_mode;
   ...
   * \param sick_availability_level Desired availability of the Sick PLS
   */
  void SickPLS::SetSickAvailability( const uint8_t sick_availability_flags )
    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::SetSickAvailabilityFlags: Sick PLS is not initialized!");
    }
    
    /* Ensure this is a valid sick sensitivity value*/
    if ( sick_availability_flags > 7 ) {      
      throw SickConfigException("SickPLS::SetSickAvailabilityFlags: Invalid availability!");
    }
	...
	throw;
      ...
      /* Handle anything else */
      	...
      	std::cerr << "SickPLS::SetSickAvailabilityFlags: Unknown exception!!!" << std::endl;
      	throw;
      }

    }
    else {
      std::cerr << "\tSickPLS::SetSickAvailability - Device is already operating w/ given availability. (skipping write)" << std::endl;
    }
    
  }

  /**
   * \brief Gets the current Sick PLS availability level flags
   * \return Sick PLS Availability level flags
   */
  uint8_t SickPLS::GetSickAvailability( ) const throw( SickConfigException ) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::GetSickAvailabilityFlags: Sick PLS is not initialized!");
    }
  ...
  /*!
   * \brief Sets the variant of the Sick PLS (scan angle and scan resolution)
   * \param scan_angle The desired scan angle of the Sick PLS
   * \param scan_resolution The desired angular resolution of the Sick PLS
   */
  void SickPLS::SetSickVariant( const sick_pls_scan_angle_t scan_angle, const sick_pls_scan_resolution_t scan_resolution )
    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::SetSickVariant: Sick PLS is not initialized!");
    }
    
    /* A sanity check to make sure that the command is supported */
    if (_sick_type == SICK_PLS_TYPE_211_S14 || _sick_type == SICK_PLS_TYPE_221_S14 ||_sick_type == SICK_PLS_TYPE_291_S14) {
      throw SickConfigException("SickPLS::SetSickVariant: Command not supported on this model!");
    }

    /* Ensure the given scan angle is defined */
    if (!_validSickScanAngle(scan_angle)) {
      throw SickConfigException("SickPLS::SetSickVariant: Undefined scan angle!");
    }

    /* Ensure the given scan resolution is defined */
    if (!_validSickScanResolution(scan_resolution)) {
      throw SickConfigException("SickPLS::SetSickMeasuringUnits: Undefined scan resolution!");
    }
    ...
    case SICK_SCAN_ANGLE_180:
      ...
      break;
    default:
      throw SickConfigException("SickPLS::SetSickVariant: Given scan angle is invalid!");
    }
    ...
    case SICK_SCAN_RESOLUTION_25:
      ...
      break;
    default:
      throw SickConfigException("SickPLS::SetSickVariant: Given scan resolution is invalid!");
    }
    
    /* Build the request message */
    message.BuildMessage(DEFAULT_SICK_PLS_SICK_ADDRESS,payload_buffer,5);
    ...
    /* Handle anything else */
      ...
      std::cerr << "SickPLS::SetSickVariant: Unknown exception!!!" << std::endl;
      throw;
    }
    ...
    if(payload_buffer[1] != 0x01) {
      throw SickConfigException("SickPLS::SetSickVariant: Configuration was unsuccessful!");
    }
  ...
  /**
   * \brief Acquires both range and reflectivity values from the Sick PLS 211/221/291-S14 (PLS-FAST)
   ...
   *       of the Sick PLS for this value to be populated.
   */
  void SickPLS::GetSickScan( unsigned int * const range_values,
	...
	throw SickIOException("SickPLS::GetSickScan: Unexpected message!");
      ...
      /* Define a local scan profile object */
      sick_pls_scan_profile_c4_t sick_scan_profile;

      /* Initialize the profile */
      memset(&sick_scan_profile,0,sizeof(sick_pls_scan_profile_c4_t));
   ...
   *       of the Sick PLS for this value to be populated.
   *
   * EXAMPLE: Using a 180/0.5 Sick PLS variant, a subrange over the scan area [0,10.5] would
   *          correspond to setting the index arguments as follows: sick_subrange_start = 1,
   *          sick_subrange_stop = 22
   */  
  void SickPLS::GetSickScanSubrange( const uint16_t sick_subrange_start_index,
    ...
    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::GetSickScanSubrange: Sick PLS is not initialized!");
    }
	...
	throw SickIOException("SickPLS::GetSickScanSubrange: Unexpected message!");
      }
      ...
      /* Define a local scan profile object */
      sick_pls_scan_profile_b7_t sick_scan_profile;

      /* Initialize the profile */
      memset(&sick_scan_profile,0,sizeof(sick_pls_scan_profile_b7_t));
	...
	*sick_telegram_index = sick_scan_profile.sick_telegram_index;
    ...
    /* Handle anything else */
      ...
      std::cerr << "SickPLS::GetSickScanSubrange: Unknown exception!!!" << std::endl;
      throw;
    }
  ...
  /**
   * \brief Returns the most recent partial scan obtained by Sick PLS
   * \param *measurement_values Destination buffer for holding the current round of measured value
   * \param &num_measurement_values Number of values stored in measurement_values
   ...
   *       Sick PLS variant to use a 0.25 degrees scan resolution.
   * 
   * NOTE: Calling this function will return either range or reflectivity measurements
   *       depending upon the current measuring mode of the device.
   *
   ...
   *       of the Sick PLS for this value to be populated.
   */  
  void SickPLS::GetSickPartialScan( unsigned int * const measurement_values,
				    unsigned int & num_measurement_values,
    ...
    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::GetSickPartialScan: Sick PLS is not initialized!");
    }
    ...
    try {
      ...
      /* Check that our payload has the proper command byte of 0xB0 */
      if(response.GetCommandCode() != 0xB0) {
	throw SickIOException("SickPLS::GetSickPartialScan: Unexpected message!");
      }
	...
	*sick_telegram_index = sick_scan_profile.sick_telegram_index;
    ...
    /* Handle anything else */
      ...
      std::cerr << "SickPLS::GetSickPartialScan: Unknown exception!!!" << std::endl;
      throw;
    }
  ...
  /**
   * \brief Returns the most recent mean measured values from the Sick PLS.
   ...
   *       of the Sick PLS for this value to be populated.
   */  
  void SickPLS::GetSickMeanValues( const uint8_t sick_sample_size,
				   unsigned int * const measurement_values,
				   unsigned int & num_measurement_values,
				   unsigned int * const sick_telegram_index,
				   unsigned int * const sick_real_time_scan_index ) throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException) {
    ...
    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::GetSickMeanValues: Sick PLS is not initialized!");
    }
    ...
    try {
      ...
      _recvMessage(response,DEFAULT_SICK_PLS_SICK_MEAN_VALUES_MESSAGE_TIMEOUT);
      
      /* Check that our payload has the proper command byte of 0xB0 */
      if(response.GetCommandCode() != 0xB6) {
	throw SickIOException("SickPLS::GetSickMeanValues: Unexpected message!");
      }
      ...
      /* Define a local scan profile object */
      sick_pls_scan_profile_b6_t sick_scan_profile;

      /* Initialize the profile */
      memset(&sick_scan_profile,0,sizeof(sick_pls_scan_profile_b6_t));
	...
	*sick_telegram_index = sick_scan_profile.sick_telegram_index;
    ...
    /* Handle anything else */
      ...
      std::cerr << "SickPLS::GetSickMeanValues: Unknown exception!!!" << std::endl;
      throw;
    }
   ...
   *          sick_subrange_stop = 22
   */  
  void SickPLS::GetSickMeanValuesSubrange( const uint8_t sick_sample_size,
    ...
    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::GetSickMeanValuesSubrange: Sick PLS is not initialized!");
    }
	...
	throw SickIOException("SickPLS::GetSickMeanValuesSubrange: Unexpected message!");
      }
      ...
      /* Define a local scan profile object */
      sick_pls_scan_profile_bf_t sick_scan_profile;

      /* Initialize the profile */
      memset(&sick_scan_profile,0,sizeof(sick_pls_scan_profile_bf_t));
	...
	*sick_telegram_index = sick_scan_profile.sick_telegram_index;
    ...
    /* Handle anything else */
      ...
      std::cerr << "SickPLS::GetMeanValuesSubrange: Unknown exception!!!" << std::endl;
      throw;
    }
  ...
  /**
   * \brief Indicates whether the device is an PLS Fast
   */
  bool SickPLS::IsSickPLSFast() const throw(SickConfigException) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickPLS::IsSickPLSFast: Sick PLS is not initialized!");
    }
    
    return (_sick_type == SICK_PLS_TYPE_211_S14 ||
  ...
  std::string SickPLS::GetSickConfigAsString( ) const {
	...
	str_stream << "\tPeak Thresh: " << SickPeakThresholdToString((sick_pls_peak_threshold_t)_sick_device_config.sick_peak_threshold) << std::endl;
	str_stream << "\tStop Thresh: " << (unsigned int)_sick_device_config.sick_stop_threshold << std::endl;
      ...
      str_stream << "\tMeasuring Mode: " << SickMeasuringModeToString((sick_pls_measuring_mode_t)_sick_device_config.sick_measuring_mode) << std::endl;
  ...
  std::string SickPLS::SickTypeToString( const sick_pls_type_t sick_type ) {
    ...
    switch(sick_type) {
    case SICK_PLS_TYPE_200_30106:
      return "Sick PLS 200-30106";
    case SICK_PLS_TYPE_211_30106:
      return "Sick PLS 211-30106";
    case SICK_PLS_TYPE_211_30206:
      return "Sick PLS 211-30206";
    case SICK_PLS_TYPE_211_S07:
      return "Sick PLS 211-S07";
    case SICK_PLS_TYPE_211_S14:
      return "Sick PLS 211-S14";
    case SICK_PLS_TYPE_211_S15:
      return "Sick PLS 211-S15";
    case SICK_PLS_TYPE_211_S19:
      return "Sick PLS 211-S19";
    case SICK_PLS_TYPE_211_S20:
      return "Sick PLS 211-S20";
    case SICK_PLS_TYPE_220_30106:
      return "Sick PLS 220-30106";
    case SICK_PLS_TYPE_221_30106:
      return "Sick PLS 221-30106";
    case SICK_PLS_TYPE_221_30206:
      return "Sick PLS 221-30206";
    case SICK_PLS_TYPE_221_S07:
      return "Sick PLS 221-S07";
    case SICK_PLS_TYPE_221_S14:
      return "Sick PLS 221-S14";
    case SICK_PLS_TYPE_221_S15:
      return "Sick PLS 221-S15";
    case SICK_PLS_TYPE_221_S16:
      return "Sick PLS 221-S16";
    case SICK_PLS_TYPE_221_S19:
      return "Sick PLS 221-S19";
    case SICK_PLS_TYPE_221_S20:
      return "Sick PLS 221-S20";
    case SICK_PLS_TYPE_291_S05:
      return "Sick PLS 291-S05";
    case SICK_PLS_TYPE_291_S14:
      return "Sick PLS 291-S14";
   ...
   */
  std::string SickPLS::SickMeasuringModeToString( const sick_pls_measuring_mode_t sick_measuring_mode ) {
  ...
  /**
   * \brief Converts Sick PLS sensitivity level to a corresponding string
   * \param sick_sensitivity Sick sensitivity level
   * \return The corresponding string
   */
  std::string SickPLS::SickSensitivityToString( const sick_pls_sensitivity_t sick_sensitivity ) {
  ...
  /**
   * \brief Converts Sick PLS peak threshold to a corresponding string
   * \param sick_peak_threshold Sick sensitivity level
   * \return The corresponding string
   */
  std::string SickPLS::SickPeakThresholdToString( const sick_pls_peak_threshold_t sick_peak_threshold ) {
   ...
   * \param &sick_device_config The desired Sick PLS configuration
   */
  void SickPLS::_setSickConfig( const sick_pls_device_config_t &sick_device_config )
    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {
    ...
    try {
      ...
      /* Set Block A */
      uint16_t temp_buffer = host_to_sick_pls_byte_order(sick_device_config.sick_blanking);
      memcpy(&payload_buffer[1],&temp_buffer,2);
      
      /* Set Block B */
      payload_buffer[3] = sick_device_config.sick_stop_threshold; // NOTE: This value will be 0 for PLS 211/221/291
      ...
      /* Set Block A3 */
      temp_buffer = host_to_sick_pls_byte_order(sick_device_config.sick_fields_b_c_restart_times);
      memcpy(&payload_buffer[31],&temp_buffer,2);
      
      /* Set Block A4 */
      temp_buffer = host_to_sick_pls_byte_order(sick_device_config.sick_dazzling_multiple_evaluation);
      memcpy(&payload_buffer[33],&temp_buffer,2);
      
      /* Populate the message container */
      message.BuildMessage(DEFAULT_SICK_PLS_SICK_ADDRESS,payload_buffer,35);
      
      /* Send the status request and get a reply */
      _sendMessageAndGetReply(message,response,DEFAULT_SICK_PLS_SICK_CONFIG_MESSAGE_TIMEOUT,DEFAULT_SICK_PLS_NUM_TRIES);
      ...
      if (payload_buffer[1] != 0x01) {
	throw SickConfigException("SickPLS::_setSickConfig: Configuration failed!");
      }
    ...
    /* Catch anything else */
      ...
      std::cerr << "SickPLS::_setSickConfig: Unknown exception!" << std::endl;
      throw;
    }
   ...
   */
  void SickPLS::_setSickOpModeMonitorStreamRangeAndReflectivity( )
    throw( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {

    /* A sanity check to make sure that the command is supported */
    if (_sick_type != SICK_PLS_TYPE_211_S14 && _sick_type != SICK_PLS_TYPE_221_S14 && _sick_type != SICK_PLS_TYPE_291_S14) {
      throw SickConfigException("SickPLS::_setSickOpModeMonitorStreamRangeAndReflectivity: Mode not supported by this model!");
    }
	...
	std::cerr << "SickPLS::_setSickOpModeStreamRangeAndReflectivity: Unknown exception!!!" << std::endl;
	throw;
      } 
   ...
   */
  void SickPLS::_setSickOpModeMonitorStreamValuesFromPartialScan( )
    throw( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
	...
	std::cerr << "SickPLS::_setSickOpModeStreamValuesFromPartialScan: Unknown exception!!!" << std::endl;
	throw;
      } 
   ...
   */
  void SickPLS::_setSickOpModeMonitorStreamMeanValues( const uint8_t sample_size )
    throw( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
	...
	throw SickConfigException("SickPLS::_setSickOpModeMonitorStreamMeanValues: Invalid sample size!");
      }
	...
	std::cerr << "SickPLS::_setSickOpModeStreamRangeFromPartialScan: Unknown exception!!!" << std::endl;
	throw;
      } 
   ...
   */
  void SickPLS::_setSickOpModeMonitorStreamValuesSubrange( const uint16_t subrange_start_index, const uint16_t subrange_stop_index )
    throw( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
	...
	throw SickConfigException("SickPLS::_setSickOpMonitorStreamValuesSubrange: Invalid subregion bounds!");
      }
      ...
      temp_buffer = host_to_sick_pls_byte_order(subrange_start_index);
      memcpy(mode_params,&temp_buffer,2);

      /* Assign the subrange stop index */
      temp_buffer = host_to_sick_pls_byte_order(subrange_stop_index);
   ...
   * \param subrange_stop_index The stopping index of the desired subrange
   */
  void SickPLS::_setSickOpModeMonitorStreamMeanValuesSubrange( const uint16_t sample_size, const uint16_t subrange_start_index, const uint16_t subrange_stop_index )
    throw( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
	...
	_sick_mean_value_sample_size != sample_size ) {
      ...
      if(sample_size < 2 || sample_size > 250) {
	throw SickConfigException("SickPLS::_setSickOpModeMonitorStreamMeanValuesSubrange: Invalid sample size!");
      }
      ...
      /* Ensure the subregion is properly defined for the given variant */
      if(subrange_start_index > subrange_stop_index || subrange_start_index == 0 || subrange_stop_index > max_subrange_stop_index) {
	throw SickConfigException("SickPLS::_setSickOpMonitorStreamMeanValuesSubrange: Invalid subregion bounds!");
      }
   ...
   */
  void SickPLS::_parseSickScanProfileB6( const uint8_t * const src_buffer, sick_pls_scan_profile_b6_t &sick_scan_profile ) const {
   ...
   * \param &sick_scan_profile The returned scan profile for the current round of mean measurements
   */
  void SickPLS::_parseSickScanProfileB7( const uint8_t * const src_buffer, sick_pls_scan_profile_b7_t &sick_scan_profile ) const {

    /* Read Block A, Sick PLS measured value subrange start index */
    sick_scan_profile.sick_subrange_start_index = src_buffer[0] + 256*src_buffer[1];

    /* Read Block B, Sick PLS measured value subrange stop index */
   ...
   * \param &sick_scan_profile The returned scan profile for the current round of mean measurements
   */
  void SickPLS::_parseSickScanProfileBF( const uint8_t * const src_buffer, sick_pls_scan_profile_bf_t &sick_scan_profile ) const {

    /* Read Block A, the sample size used in computing the mean return */
    sick_scan_profile.sick_sample_size = src_buffer[0];

    /* Read Block B, Sick PLS measured value subrange start index */
    sick_scan_profile.sick_subrange_start_index = src_buffer[1] + 256*src_buffer[2];

    /* Read Block C, Sick PLS measured value subrange stop index */
   ...
   * \param &sick_scan_profile The returned scan profile for the current round of measurements
   */
  void SickPLS::_parseSickScanProfileC4( const uint8_t * const src_buffer, sick_pls_scan_profile_c4_t &sick_scan_profile ) const {
  ...
  /**
   * \brief Indicates whether the Sick is an PLS 200
   * \return True if the device is a Sick PLS 200, False otherwise
   */ 
  bool SickPLS::_isSickPLS200( ) const {
  ...
  /**
   * \brief Indicates whether the Sick is an PLS 211
   * \return True if the device is a Sick PLS 211, False otherwise
   */ 
  bool SickPLS::_isSickPLS211( ) const {
  ...
  /**
   * \brief Indicates whether the Sick is an PLS 220
   * \return True if the device is a Sick PLS 220, False otherwise
   */ 
  bool SickPLS::_isSickPLS220( ) const {
  ...
  /**
   * \brief Indicates whether the Sick is an PLS 221
   * \return True if the device is a Sick PLS 221, False otherwise
   */ 
  bool SickPLS::_isSickPLS221( ) const {
  ...
  /**
   * \brief Indicates whether the Sick is an PLS 291
   * \return True if the device is a Sick PLS 291, False otherwise
   */ 
  bool SickPLS::_isSickPLS291( ) const {
   ...
   */ 
  bool SickPLS::_validSickSensitivity( const sick_pls_sensitivity_t sick_sensitivity ) const {
   ...
   * \param sick_peak_threshold Peak threshold definition for Sick PLS
   */ 
  bool SickPLS::_validSickPeakThreshold( const sick_pls_peak_threshold_t sick_peak_threshold ) const {
   ...
   * \param availability_flags The availability level of the Sick PLS
   * \return The corresponding string
   */
  std::string SickPLS::_sickAvailabilityToString( const uint8_t availability_flags ) const {
   ...
   * \return The corresponding string
   */
  std::string SickPLS::_sickRestartToString( const uint8_t restart_code ) const {
    ...
    case 0x04:
      ...
      break;
    case 0x05:
      restart_str += "PLS operates as a slave, restart after set time";
      break;
    case 0x06:
      restart_str += "PLS operates as a slave, immediate restart";
      break;
    default:
      restart_str += "Unknown!";      
    }
  ...
  /**
   * \brief Converts Sick PLS temporary field code to a corresponding string
   * \param temp_field_code The temporary field code
   * \return The corresponding string
   */
  std::string SickPLS::_sickTemporaryFieldToString( const uint8_t temp_field_code ) const {
  ...
  /**
   * \brief Converts Sick PLS subtractive fields code to a corresponding string
   * \param subt_field_code The subtractive fields code
   * \return The corresponding string
   */
  std::string SickPLS::_sickSubtractiveFieldsToString( const uint8_t subt_field_code ) const {
  ...
  /*!
   * \brief Converts Sick PLS contour function code to a corresponding string
   * \param contour_function_code The subtractive fields code
   * \return The corresponding string
   */
  std::string SickPLS::_sickContourFunctionToString( const uint8_t contour_function_code ) const {
  ...
  /**
   * \brief Converts the Sick PLS variant to a corresponding string
   * \param sick_variant The Sick PLS variant
   * \return The corresponding string
   */
  std::string SickPLS::_sickVariantToString( const unsigned int sick_variant ) const {

    /* Return the correct string */
    if(sick_variant == SICK_PLS_VARIANT_2XX_TYPE_6) {
      return "Standard device (PLS,type 6)";
    }
    else if (sick_variant == SICK_PLS_VARIANT_SPECIAL) {
      return "Special device (PLS211-/221-S19/-S20)";
    }
    else {
      return "Unknown";
    }
